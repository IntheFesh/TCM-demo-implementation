...
| SDT 适配器 | 18 条 | 格式验证 0 调用 + 5 条 prompt 质量冒烟 | Test 满分提交得 50.0000/50，格式逐字节正确 |
...
- 安全否决两个数都报，句式：「本系统 SDT 得分为 X/50，其中 8 条（16%）因触发
  ...
  代价，我们选择保留它。」
- **先看 Task1 的逐字命中率**。它按字符串完全相等评分，要的是原文片段，跟我们
  S1 的术语归一化方向相反。命中率若明显低于其他三项，先改 `prompts/v1/sdt_extract.yaml`
  再跑全量，别在全量上烧钱
