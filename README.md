...
## 五组消融（R57/R61）：证明不靠模仿也能推

R38 消融的是生成流水线的工程旋钮（五家融合/best-of-N/S1+S2 合一），四组都走
`S3_MODE=structured`。这一轮消融的是另一条轴——**要不要检索医案才能推出
正确结论**，前四组沿着"有没有医理规则层""医案进不进推导""有没有事后佐证"
这三个布尔展开，**定义只在 `eval/ablation/spec.py` 一处**：

| 组 | 医理层 | 医案进推导 | 事后佐证 | 回答什么 |
|---|---|---|---|---|
| A 现状模仿机制 | 关 | 开 | 关（该相不跑） | 基线：不靠医理规则、靠检索医案驱动生成 |
| B 既无医理也无医案 | 关 | 关 | 关 | 去掉模仿之后，连医理规则都不给，模型能推到什么程度 |
| C 纯演绎（核心组） | 开 | 关 | 关 | **不靠模仿也能推**的直接证据——医理规则驱动、零医案 |
| D 演绎+事后佐证（最终形态） | 开 | 关 | 开 | 完整产品形态：演绎推导 + 事后医案佐证 |
| E 纯演绎复测（噪声地板对照） | 开 | 关 | 关 | R61 新增，配置逐一同 C——量"两次独立采样天然有多少分歧" |

**硬指标**（`eval/ablation/spec.py::GATE_*`）：
1. C 组验证器一次过率 ≥ A 组；
2. C 组 `rule_refs` 完整率（推导链上每一步真的挂着医理规则的占比）≥ 0.9；
3. C 与 D 的证型/主方/治法**三项分开判**（R61：不再合成一个布尔，也不再
   跟绝对值 0.9 比）——证型、主方各自归一后判一致率，治法（自由文本）
   判字符级相似度均值，三项都要求"不明显低于 C-E 噪声地板"（margin=0.1）
   才算过。C/D 唯一差异是要不要跑第三相事后佐证，这条验的正是 R54"绝不
   回头改推导"这条不变式：加不加事后佐证，结论不该系统性地变。**不达标
   就回 R51 补规则，不许把医案放回推导相凑数**——那是回退，不是修复。

```bash
python -m eval.ablation.r57 --backend fake --sdt-dir <TCMEval>/evaluation/TCMEval-SDT --limit 2
python -m eval.ablation.r57 --backend real --sdt-dir <TCMEval>/evaluation/TCMEval-SDT
```

`--sdt-dir` 给了就自动按"证型第一个词含脾/胃"从 SDT Train 里挑 20 条
（判据与排序规则见 `eval/ablation/r57.py::select_pi_wei_men_complaints`
的文档字符串，选出来的 20 条病案号是 35/53/117/118/135/191/205/279/286/
316/346/349/6/41/47/49/85/104/106/150，确定性可复现）；没有 SDT 数据集时用
`--queries-path` 传一份自备的主诉文件。

**这个沙盒没有真实 LLM 后端，跑不出真机数**——`--backend fake` 只验证了
管道本身（各组能不能各自设对环境变量、跑通、不崩、报告格式对不对；这个
过程中修了两处 `scripts/bench_consult.py` 里此前从未被 `S3_MODE=derived`
真正跑过的假设：假后端生成的 `S3Derived` 不满足"每步要么引规则要么标
insufficient"这条模型校验器，以及 `invalid_reason()` 把"这一相不检索医案
（0 位医家）"错判成"没有产出结论"）。真机 20 条 × 5 组（R61 加了 E 组
噪声地板对照）= 100 次问诊——**单次墙钟没有可信数字可引用**：R55 commit
留下两处互相矛盾的记录（"45~75 秒一档" vs 另一处
"264.6/276.3/208.9 秒"，量级差 3~4 倍），量出后者的工具
`scripts/compare_reasoning_tiers.py` 不存在于仓库，无法核实哪个准。
**跑法、断点续跑、结果判读、失败诊断的完整操作手册见
`docs/ONSITE_R57_R58.md`**，其中第一步就是用 `--limit 2` 跑 10 次问诊
拿到本机真实耗时再估算完整 100 次的时长与费用，不在这里给一个可能误导
的数字。
...
量级远小于 R57 的 80 次问诊）。**R57/R58 完整操作手册（命令、断点续跑、
结果判读、失败诊断）见 `docs/ONSITE_R57_R58.md`**，不在这里重复。
...
无构建步骤，`web/index.html` + `app.css` + `app.js` + `graph.js` + `ui/select.js`，两个页签：

### R24 的八项改造（一屏之内的"这不是一个模板"）

| # | 项 | 一句话 |
|---|---|---|
| 1 | **自绘下拉**（`web/ui/select.js`） | 原生 `<select>` 的弹出层由操作系统画、不吃页面字体。自绘层**包在原生元素外面**，原生元素仍是值的唯一来源（读值的代码一行没改） |
| 2 | **首屏题记** | 三行：这是什么 / 给什么 / 不给什么。**写项目自己的话，不引古人**——引文必须能核对到原文，而 `books/` 是 gitignore 的，编一句假出处比不放题记糟得多。也**不写医家数**（注册表可变，写死就是第二处实现） |
| 3 | **字体子集化**（`scripts/subset_fonts.py`） | 字表从仓库真实文本收集（1817 个字符），不手写。**医案原文默认不收**（十几万字，收了等于没子集化），代价写在脚本文档里，`--include-cases` 是那条开关 |
| 4 | **对照带改 SVG + 斜纹** | 带高 3px → 18px；ε 段画成**斜纹**不是第二种灰——纹理不依赖亮度，投影仪压暗了也还在。`bandSegments` 的算法一个字没改，改的是画法 |
| 5 | **四级列层次 + 君臣佐使两列密排** | 证型（宋体大字）/ 治法（换字族）/ 方名（加界线）/ 药味（缩进两列）四级**各用一种手段**，不是同一手段的四个刻度。药味左边界对齐，竖着扫一眼就知道君臣各几味 |
| 6 | **去卡片化** | 内容块从"圆角边框盒"改成"左侧标记线 / 上分隔线 + 留白"。控件（按钮、输入框、chip、浮层）保留圆角——它们本来就该看起来可点 |
...
（"在 DOM 里"和"看得见"是两件事）。截图本身也当场看出两处：建议行把药名渲染了
两遍（reason 里已经点过名）、评分 `0` 和 `0.7` 不等宽。
...
这张图当前的规模：节点 2356 `../data/graph.json:graph.n_nodes=2356`、
...
**这四个数连着两轮在降**（R28 1315/3771/178/1117 → R29 1312/3765/177/1115 →
R31 1281/3682/174/1087），两轮降的都是**同一个东西被拼错/错名成两份**的那部分：
R29 是 OCR 修正表覆盖到 name/disease 两列（「温疤证」并回「温疟证」、
「唇甲发」并回「唇甲发绀」）；R31 是把扫描件丢了行首 `#` 的证型标题认出来，
33 条条目拿回了自己的名字——其中 3 条（TB-013/098/183）**本来只是因为顶着
上一条的错名（含「肝」「脾」）才落进这张图的脾胃肝子集**，名字改对之后
它们的病位其实是肺/心/胆，退出子集是这张图变更准而不是变小了。
...
一位医家的全量医案 ≈ 180K token：**首次** $0.24，之后每次 $0.008。

**一次问诊到底多少钱：⏳ 还没实测，下面是算式。** 这一格以前写的是「≈ ¥0.17」，
那个数只算了**命中的输入**（540K × $0.044/M × 7.2 = ¥0.171，一分不差），
漏了输出——R22 之后 S3 默认 `reasoning_effort=max`，**思考 token 按输出价计费**
（$3.96/M，是命中输入价的 90 倍）。按默认配置重算：

| 项 | 算式 | 估值 |
|---|---|---|
| S3 调用数 | 3 位医家 × `S3_BEST_OF_N=3` | **9** 次 |
| 每次 S3 | 180K 命中输入 + 约 3600 输出 token | ≈ **¥0.16** |
| 一次问诊 | 9 × ¥0.16（+ S1/S2 两次小调用） | ≈ **¥1.44**（高峰价；谷段五折 ≈ ¥0.72） |
| `FAST_MODE=1` | S3 降到 3 次（`S3_BEST_OF_N=1`） | ≈ **¥0.48**（谷段 ≈ ¥0.24） |

**这三个数都是估算，不是账单**：3600 那个输出 token 数是假设
（见 `offline/distill_from_v4.py` 的 `OUT_TOKENS_PER_CALL_ESTIMATE`）。
**实测以 `/api/usage` 的 `tokens_today` 为准**，跑完一次问诊就能看到真实的
hit/miss/输出三个数。在那之前，这一行在 `eval/RESULTS.md` 里标 ⏳。

缓存是**默认开的、不需要改代码**，但有三条官方规矩决定了实现方式
