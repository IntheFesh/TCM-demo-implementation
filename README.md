...
正确结论**，四组沿着"有没有医理规则层""医案进不进推导""有没有事后佐证"
...
**三条硬指标**（`eval/ablation/spec.py::GATE_*`）：
1. C 组验证器一次过率 ≥ A 组；
2. C 组 `rule_refs` 完整率（推导链上每一步真的挂着医理规则的占比）≥ 0.9；
3. C 与 D 的证型/治法/主方**一致率** ≥ 0.9——C/D 唯一差异是要不要跑第三相
   事后佐证，这条验的正是 R54"绝不回头改推导"这条不变式：加不加事后佐证，
   结论不该变。**不达标就回 R51 补规则，不许把医案放回推导相凑数**——
   那是回退，不是修复。
...
管道本身（四组能不能各自设对环境变量、跑通、不崩、报告格式对不对；这个
...
（0 位医家）"错判成"没有产出结论"）。真机 20 条 × 4 组 = 80 次问诊——
**单次墙钟没有可信数字可引用**：R55 commit 留下两处互相矛盾的记录
（"45~75 秒一档" vs 另一处"264.6/276.3/208.9 秒"，量级差 3~4 倍），
量出后者的工具 `scripts/compare_reasoning_tiers.py` 不存在于仓库，
无法核实哪个准。**跑法、断点续跑、结果判读、失败诊断的完整操作手册见
`docs/ONSITE_R57_R58.md`**，其中第一步就是用 `--limit 2` 跑 8 次问诊
拿到本机真实耗时再估算完整 80 次的时长与费用，不在这里给一个可能误导
的数字。
