...
已有的报告文件）——80 次问诊里断在第三组，重跑整条命令等于前两组的钱和时间
全部重花。真正的续跑方式是**按组分别跑、分别存盘**：
...
按组取出来，**重新跑一遍 `aggregate()`/`build_report()`**，这样三条硬指标
是在合并后的完整四组数据上算出来的，不是四份独立算好的半成品拼在一起
（半成品拼接会漏掉 `pair_consistency` 这类需要同时看 C 和 D 两组原始行的
计算）。

**四份局部文件必须来自同一份 `complaints`**（同一次 `--sdt-dir`/
`--queries-path` 选出的同 20 条主诉，`record_id` 要能一一对上）——不检查
这件事就合并，等于拿四组不同主诉的结果硬凑成"一次消融实验"，C vs D 的
配对比较会静默地把不同主诉的结果错配在一起。这个脚本在合并前校验
...
def _check_same_complaints(reports: list[tuple[Path, dict]]) -> list[dict]:
    """四份局部报告必须出自同一批主诉，否则 C/D 配对比较会错配。"""
...
def merge(paths: list[Path]) -> dict:
    ...
    missing = [g for g in "ABCD" if g not in rows_by_group]
    if missing:
