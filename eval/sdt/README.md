...
**这句话 R10 已经反过来写了（2026-09-15，Test 50 条实测）。** 实测结果：

    task2  多选 15/50（30.0%）  少选 21/50（42.0%）
    task3  多选  2/50（ 4.0%）  少选 24/50（48.0%）
    合计   多选率 17.0%  少选率 45.0%
    边际代价（官方 score_proportional 的反事实）：
      去掉全部错选 +2.77 分 / 补上全部漏选 +14.11 分
      单个错选 +0.0544  vs  单个漏选 +0.1425  → 一个漏选贵 2.6 倍
    判据：少选是主因（14.11 vs 2.77，过 0.5 分绝对门槛、过 1.5× 比值门槛）

按上面这张表的第二行，`prompts/v1/sdt_select.yaml` 那句话**已经改成**「漏选比选错
更贵，拿不准的也要选上」。**下一步必须先在 Train 上验证方向**（Test 已经跑过 4 次，
台账 `test_run_log.jsonl`）：

```bash
python -m eval.sdt.run --sdt-dir "$SDT" --split Train --solver chain --out out/sdt_train_a.txt
# 改 prompt 前后各跑一次 Train，比两次的分；方向对了再考虑动 Test
```

**R10 之前那句话（留作记录）：** `prompts/v1/sdt_select.yaml` 从第一版到 R10 写的是
