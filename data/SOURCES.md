...
39. **提 SDT 的第一轮（R2）：先把失分拆开看，**并且**发现 R2-2 那条 prompt 改动
    ...
    **一处待核对的文档不一致（这一轮没有替它选一个数）：** `eval/RESULTS.md`
    第 5 行写「被拦的 10/50 条全部是真危重」，本文件第 14 条的确定性统计是
    Test 8/50（16%）。两个数出自不同的地方、都被引用过，需要在真机上重新数一次
    才能定哪个对——回填台账时把这个不一致写进了那条记录的 `note`，没有悄悄
    按其中一个数记。
