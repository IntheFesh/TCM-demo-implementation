...
- S1（症状标准化）全局只跑一次，两位医家共用结果。如果对每位医家各跑
  ...
  指向不存在的节点。
- 新增的 `.jsonl` 数据文件一律放 `data/standard/`，不要放别处。`.gitignore`
  对 `*.jsonl` 是整体忽略、只对 `data/standard/*.jsonl` 开了例外——这个坑
  已经踩过两次：`data/standard/syndromes.jsonl` 当初差点被静默吞掉，M4 建
  `data/diseases.jsonl`（任务描述原文给的路径）时又落在例外之外，同一个
...
- **涉及图层结构变更（layer 编号、新增/合并层、compound 节点）时，
  ...
  类问题（它们根本不会调用前端渲染代码）。只有真的把数据喂给浏览器跑一遍
  才会暴露。凡是改了 layer 数量/含义、加了 compound 关系，都要跑一次
  Playwright（真实数据、真实 DOM、真实 cytoscape 渲染），不能只靠后端
  JSON 测试通过就认为前端也对。
