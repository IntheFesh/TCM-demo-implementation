...
def test_only_the_costly_segments_get_the_warning():
    ...
    **有意的契约变更（R25 / R26）**：期望值从 5/6/7/8 先变成 5/6/7/8/9（R25 的
    第 9 段估 320 次真实调用），再变成 5/6/7/8/9/10（R26 的蒸馏段，按 ¥40 预算
    估 350 次）。每次都必须改期望值、不能改成"只要 5 在里面就算过"——后者会让
    这条测试从此再也发现不了"某段花钱但没被登记进 COSTLY_SEGMENTS"这类漏登记。
    """
    src = (ROOT / "scripts" / "run_onsite.sh").read_text(encoding="utf-8")
    line = next(ln for ln in src.splitlines() if ln.startswith("COSTLY_SEGMENTS="))
    assert line.split("=", 1)[1].strip().strip('"').split() == ["5", "6", "7", "8", "9", "10"]
