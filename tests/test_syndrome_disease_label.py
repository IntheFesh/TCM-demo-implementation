...
177 个证候节点里 **67 个重名**（26 个名字重复 2–4 次；R28 当时是 178 个节点
里 66 个重名 / 25 个名字，R29 修 OCR 表之后「温疤证」并回「温疟证」，
节点少了一个而重名名字多了一个——名字并进来之后它跟原有的「温疟证」撞上了）。「肝郁气滞证」在
...
def test_the_still_ambiguous_pairs_are_a_data_defect_not_a_display_bug():
    ...
    加病名限定之后仍有 15 组 (名字, 病名) 撞在一起。**R29 把根因逐条打出来了，
    R28 这条注释原来的归因（「名字列跟定义列错位」）只对其中一类，而且描述错了
    机制**——不是两列错位，是同一个名字被下一条沿用。四类：

      1. **编号标题掉了行首的 `#`**（主导，教材原文实测：带 `#` 的编号标题 237 个、
         不带的编号行 934 个）。`_SYN_HEADING_RE` 锚在 `#` 上，认不出
         `7.痰火扰心`（原文第 3001 行）这种，于是上一条的名字一直沿用。
         TB-114/115/116 三条都叫「热证」就是这么来的。
      2. **OCR 形近字**：`disease=黄疽`→黄疸、`name=温疤证`→温疟证。
         **R29 已修**（`data/standard/ocr_fixes.tsv` 加了 scope 列）。
      3. **教材原文本身掉字**：`# 6.阻心脉`（原文第 2981 行，少「瘀」）、
         `# 第五节 逆`（第 5968 行，少「呃」）、`# 第三节   闭`（第 8737 行，少「癃」）。
         补哪个字要靠语义猜——只报不改，进
         `build_syndrome_textbook --report` 的「可疑条目」清单。
      4. **名称错切/合并**：`（6）哮喘脱证`（原文第 1781 行）。同上，只报不改。

    **这个数只能降不能升。** 降了：把这一行的 15 改成新的实测值，并在
    docs/reports/R29_report.md 里记一笔它为什么降。升了：解析器退化了，
    去查哪一步把名字弄丢了，**不是把这一行改成 `<=`**。
    R29 重新生成证候表与图谱之后它仍然是 15——两组被解开的重名
    （TB-102 阳痫证→阴痫证、TB-185 出血证→神昏证）落在图里合并过的节点上，
    没有改变图层的组数。
    """
    from api.main import ambiguous_syndrome_keys
