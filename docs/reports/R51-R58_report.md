...
| 数字 | 值 | 对照基准 |
|---|---|---|
| 三档墙钟实测（top3/full_context/derived 经验值） | 264.6 / 276.3 / 208.9 秒 | 三档顺序落在测量噪声内，不支持"档位越高越慢" |
...
- **三档真机质量对照表**（证型/主方/验证器一次通过率/rule_refs 完整率）：
  沙盒没有真实 LLM 后端，跑不出这份数字，工具已就位在
  `scripts/compare_reasoning_tiers.py`，需要用户在自己的机器上跑。
...
**成本量级**：20 条 × 4 组 = 80 次问诊，按 R55 记录的单次问诊墙钟量级
（top3 档约 45 秒、full_context 档约 75 秒，derived 模式经验上接近 top3
档）估算**约 1~1.5 小时机器时间**；按当前主流 API 价格量级估算**约 ¥5**——
这两个数字需要用户在自己的机器上实测确认，此处只给量级。跑完后
`eval/report_ablation_r57.json` 的 `gates`/`all_gates_passed` 字段就是
三条硬指标的判定结果。
