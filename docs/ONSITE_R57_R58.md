...
**R61 新增了 E 组**（C 组的噪声地板复测，三个开关跟 C 逐一相同）——不是
任务书原文的第五种实验条件，是量"两次独立采样天然会有多少分歧"用的
对照，C-D 一致率现在跟它比，不跟绝对值比（见下面 1.5）。所以下面所有
"四组"都变成"五组"、"80 次"都变成"100 次"。
...
`--limit 2` 只跑 2 条主诉 × 5 组 = 10 次真实问诊，几分钟到二十分钟内应该
跑完。跑完看终端最后打出的表格，或者：
...
会不一样，一般 D 组（演绎+事后佐证）最慢；E 组配置跟 C 组相同，耗时应该
跟 C 组接近。

### 1.2 第二步：用真实数字估算完整 100 次的时间和费用

完整跑法是 20 条主诉 × 5 组，各组耗时不同，粗略按"五组平均墙钟 × 100"
估算总耗时：

```
总耗时（秒）≈ (A组秒数 + B组秒数 + C组秒数 + D组秒数 + E组秒数) / 5 × 100
```

比如五组分别是 60/55/70/90/68 秒，总耗时 ≈ (60+55+70+90+68)/5 × 100 ≈
6860 秒 ≈ 1.9 小时。**这是从你自己机器、自己网络条件下现场量出来的数字，
不是猜的**——这正是不信任历史数字、自己重新量一遍的意义。

费用：`/tmp/r57_trial.md` 或 `/tmp/r57_trial.json` 里 `groups.*.llm_calls_mean`
是每次问诊平均调用几次模型。10 次问诊（--limit 2）的真实花费可以在你的
DeepSeek 账户后台账单里直接看到（几分钟内会更新），拿这个真实花费除以 10
再乘以 100，就是 100 次的费用估算——比套用任何写死在文档里的单价更准，
因为不同 prompt 长度、不同推理档位实际花费差异很大，账单是唯一的真话。

### 1.3 第三步：正式跑——推荐按组分开跑，不要一条命令跑完五组

一条命令跑完 ABCDE 五组，中途断了（断网、断电、AutoDL 实例被抢占）**没有
断点续跑**——`eval.ablation.r57` 每次调用只把这次跑的组写进报告文件，
不会读、不会合并磁盘上已有的文件。100 次问诊按 1.5~2.5 小时算，断的概率
不是零，一条命令跑完损失的是全部已完成的进度。
...
python -m eval.ablation.r57 --backend real \
    --sdt-dir TCMEval/evaluation/TCMEval-SDT \
    --groups E --out eval/report_ablation_r57_E.json
```

每条命令跑的是 20 次问诊（一组），耗时是总时间的 1/5。**哪一组断了就只
...
这一步不是简单拼 JSON 文本——它会把五份文件里每条主诉的原始结果重新
汇总一遍，算出完整的硬指标（尤其是 C 与 D 的一致率，以及 C 与 E 的噪声
地板，这两个数都必须同时看到两组的原始数据才能算，单独一份文件里没
有）。如果五份文件不是同一批主诉跑出来的（比如中途换了 `--sdt-dir`），
这一步会直接报错拒绝合并，不会给你一份看起来正常、实际张冠李戴的假
报告。
...
`n_fail` 而言）没有问诊失败，不直接等于硬指标都过——**判读以
...
**C 与 D 一致率不达标**（不是 rule_refs 完整率或验证器通过率）：R61 起
这条门判的是"C-D 的分歧是否明显超过 C-E 噪声地板"，不是绝对值——先看
`eval/report_ablation_r57.md` 里"C-D 一致率 vs C-E 噪声地板"这张表，
三项（证型/主方/治法）哪一项差得远。诊断脚本帮不上忙这一条——C 与 D
的差异是要不要跑第三相事后佐证，理论上不该影响结论。看
`eval/report_ablation_r57.json` 里 `c_vs_d_consistency.pairs` 字段（每条
配对的证型/主方/治法原文都在里面），把 `pairs` 里三项对不上的
`record_id` 连同 `c_vs_noise_floor`（同一份配对形状，是 C-E 的地板数字）
和 `eval/report_ablation_r57.md` 一起发给开发方，这属于代码逻辑问题，
不是规则库覆盖面问题。
